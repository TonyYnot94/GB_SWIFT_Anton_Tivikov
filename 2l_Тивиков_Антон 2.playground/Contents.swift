...
var arrayErat = [Int](2...100)

for divid in arrayErat{
    for elem in arrayErat {
        if elem % divid == 0 && elem != divid {
            arrayErat.remove(at: arrayErat.firstIndex(of: elem)!)
        }
    }
    
    if arrayErat.count > 100{
        arrayErat.removeLast()
    }
